...
const Dashboard = () => {
  ...
  const chart2 = {
    ...
    options: {
      ...
      plotOptions: { bar: { horizontal: true, borderRadius: 4, barHeight: "20%" } },
  ...
  // ======================
  // Gráfico combinado de Feedbacks
  // ======================
  const feedbackChart = {
    series: [75, 25], // [Positivos, Negativos]
    options: {
      ...
      colors: ["#27993aff", "#b80a0aff"], // verde para positivos, vermelho para negativos
  ...
  const [areaChart, setAreaChart] = useState({
    ...
    options: {
      ...
      annotations: {
        yaxis: [{ y: 30, borderColor: '#999', label: { show: true, text: 'Support', style: { color: "#fff", background: '#00E396' } } }],
        xaxis: [{ x: new Date('14 Nov 2012').getTime(), borderColor: '#999', yAxisIndex: 0, label: { show: true, text: 'Rally', style: { color: "#fff", background: '#775DD0' } } }]
      }
    },
    selection: 'one_year'
  });

  const updateAreaChart = (timeline) => {
    setAreaChart(prev => ({ ...prev, selection: timeline }));
    switch (timeline) {
      case 'one_month':
        ...
        <div className="dashboard-grid">
          ...
          <div className="dash-card area-card">
            ...
            <div className="toolbar">
              <button onClick={() => updateAreaChart("one_month")}>1M</button>
              <button onClick={() => updateAreaChart("six_months")}>6M</button>
              <button onClick={() => updateAreaChart("one_year")}>1Y</button>
              <button onClick={() => updateAreaChart("ytd")}>YTD</button>
              <button onClick={() => updateAreaChart("all")}>ALL</button>
            </div>
            <ReactApexChart
